...
static void* hw_thread_loop(void *arg) {
    ...
    // Inicjalizacja ekranu 128x64 (zgodnie z Twoim fizycznym ekranem)
    ...
    while (1) {
        ...
        // Debug: Napis na samej górze (Y=10 przy obrocie 90 stopni to margines od krawędzi)
        ssd1306_framebuffer_draw_text_extra(fbp, "BROWSER", 5, 10, 8, SSD1306_FONT_DEFAULT, 4, opts, 2, &bbox);
        ...
        for (int i = 0; i < max_lines; i++) {
            ...
            // Startujemy od 25 (pod nagłówkiem), każda kolejna linia co 10 pikseli
            int y_pos = 25 + (i * 10);
            
            // X=5 (lekki margines od bocznej krawędzi)
            ssd1306_framebuffer_draw_text_extra(fbp, line_buf, 5, y_pos, 8, SSD1306_FONT_DEFAULT, 4, opts, 2, &bbox);
        }
